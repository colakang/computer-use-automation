...
# Pixel-only prototype (VisionSurface): same approved artifact, perceived through a pinned
# screen parser (VLM). Needs a model for *perception*, so it is opt-in:
#     uv run python scripts/make_evidence.py vision
VISION = ["--surface", "vision"]
VISION_SCENARIOS: dict[str, tuple[str, list[str], list[str] | None]] = {
    "vision-happy": ("Pixels only: DOM-recorded artifact replayed from screenshots + mouse/keyboard",
                     [BAL, "-t", "harbor", "-i", "member_id=10042", *VISION], None),
    "vision-not-found": ("Pixels only: business outcome detected from on-screen text",
                         [BAL, "-t", "harbor", "-i", "member_id=99999", *VISION], None),
    "vision-interstitial": ("Pixels only: System Notice recognised and dismissed via the same app-profile handler",
                            [BAL, "-t", "harbor", "-i", "member_id=10042", "--inject", "interstitial=true", *VISION], None),
    "vision-obscured": ("Pixels only: unknown popup over the target; the click lands on the overlay → caught at the right step because the screen did not change",
                        [BAL, "-t", "harbor", "-i", "member_id=10042", "--inject", "survey_modal=true", *VISION], None),
}
SCENARIOS_ALL = SCENARIOS | VISION_SCENARIOS


def run(name: str) -> dict:
    desc, args, bot = SCENARIOS_ALL[name]
    bot_proc = None
    if bot:
...
def main() -> None:
    if sys.argv[1:] == ["vision"]:
        rows = [run(n) for n in VISION_SCENARIOS]
        _index(rows, EVID / "VISION.md", "# Pixel-only replay evidence (VisionSurface prototype)\n\n"
    ...
    if sys.argv[1:]:
        return
    _index(rows, EVID / "REPLAYS.md", "# Replay evidence index\n\n"
...
def _index(rows: list[dict], path: Path, intro: str) -> None:
    lines = [
        intro,
